...
## D-10. Open Graph 공유 이미지·파비콘 제작 방식
- **결정**: 카카오톡 공유용 OG 이미지(1200×630)와 파비콘(512/180/32px PNG)을 HTML/CSS로 디자인한 뒤 헤드리스 Chrome으로 렌더링해 생성. og:image는 절대 URL로 지정.
- **이유**: 카카오톡 스크래퍼는 SVG를 지원하지 않고 절대 URL의 PNG/JPG를 요구함. HTML 렌더링 방식은 브랜드 토큰(다크 네이비 #041423 계열 + TILON Green)을 CSS로 그대로 재사용할 수 있고, 문구 수정 시 재렌더링만 하면 됨. 요청 문구 "AI Native Campus 제안"을 헤드라인으로 사용.
